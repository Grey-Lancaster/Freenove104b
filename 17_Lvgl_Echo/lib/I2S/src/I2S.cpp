...
void I2SClass::_rx_done_routine(){
  size_t bytes_read = 0;
  // BUG FIX (vendored copy of the framework's libraries/I2S/src/I2S.cpp,
  // project-local override so it doesn't touch the shared PlatformIO
  // package cache and stays portable/git-tracked): the stock version of
  // this line omits the *2-for-stereo factor its _tx_done_routine()
  // counterpart has, so it only drains half of each stereo DMA transfer
  // into the ring buffer per RX_DONE event -- the other half gets
  // overwritten by the next DMA cycle before anything comes back for it,
  // i.e. genuine permanent loss of roughly every other segment of real
  // audio, not just slower delivery (confirmed: waiting longer for more
  // bytes didn't recover normal-sounding audio, it just took longer while
  // still sounding equally sped-up/compressed). The temp buffer allocated
  // right below is already sized _i2s_dma_buffer_size*4 == this corrected
  // value (bitsPerSample/8=2, *2=4), confirming this *2 was the original
  // intent and simply got dropped from this RX copy of the calculation.
  const size_t single_dma_buf = _i2s_dma_buffer_size*(_bitsPerSample/8)*2;
