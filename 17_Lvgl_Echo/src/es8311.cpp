...
esp_err_t es8311_codec_init(void)
{
    ...
    // This was left commented out (copied from 07_Echo, which never enabled
    // it either) -- it's a separate digital gain stage from the PGA gain
    // es8311_microphone_config() already sets, and leaving it unset is why
    // 17_Lvgl_Echo's recordings came back essentially inaudible. The sibling
    // `translate` project (github.com/Grey-Lancaster/translate) hit and
    // documented this exact issue on the same codec: even 24DB wasn't
    // enough to pick up a quieter voice, needed the max, 42DB.
