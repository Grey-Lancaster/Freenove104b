...
esp_err_t es8311_codec_init(void)
{
    ...
    // Freenove's original code declares global_es_handle but never assigns
    // it, so es8311_set_mic_gain()/es8311_set_voice_mute() silently no-op
    // (both guard on `if(global_es_handle)`). Assigning it here is what the
    // rest of the file's structure clearly intends.
    global_es_handle = es_handle;
